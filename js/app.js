...
function drawResults(callback) {
  ...
  $.getJSON('https://api.foursquare.com/v2/venues/explore?ll=' + loc["lat"] + ',' + loc["lon"] + '&section=coffee&limit=50&radius=8000&sortByDistance=1&venuePhotos=1&client_id=' + CLIENT_ID + '&client_secret=' + CLIENT_SECRET + '&v=20161115',
  function(data) {
    results = $.grep(data.response.groups[0].items, function(recommendation) {
      return recommendation.venue.categories[0].id === '4bf58dd8d48988d16d941735' || recommendation.venue.categories[0].id === '4bf58dd8d48988d1e0931735';
    }).slice(0, 10);
    // if(results.length !== 0) {
    ...
    $.each(results, function(i, recommendation){
      ...
      directionsUrl = 'https://www.google.com/maps/dir/' + loc.lat + ',' + loc.lon + '/' + lat + ',' + lon;
      content = $('<li class="listing"><div class="venue"><div class="photo" style="background:url(' + photos.prefix + 'width400' + photos.suffix + ') center center"><h1>' + recommendation.venue.name + '</h1></div><a href="' + directionsUrl + '" class="directions">Go</a><div class="details"><address>' + recommendation.venue.location.address + '<br>' + recommendation.venue.location.city + ', ' + recommendation.venue.location.state + ' ' + recommendation.venue.location.postalCode + '</address></div></div></li>');
      content.data('recommendation', i);

      recommendation.venue.marker = L.marker([lat, lon]).addTo(map);
      recommendation.venue.marker.bindPopup('<strong>' + recommendation.venue.name + '</strong><br><a href="' + directionsUrl + '" class="directions-text">Get directions here</a>');
...
function randomizeBg() {
  ...
  $('.search').css({'background-image': 'url(img/' + images[Math.floor(Math.random() * images.length)] + ')'})
};
